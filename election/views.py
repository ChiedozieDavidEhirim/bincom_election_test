...
def polling_unit_results(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'election/polling_unit_results.html', context=context)
    else:
        form = PollingUnitResultsForm()
    context = {
        'form':form
    }
    return render(request, 'election/pu_results_form.html', context=context)
...
def polling_units_results_sum(request):
    return render(request, 'election/polling_units_results_sum.html')